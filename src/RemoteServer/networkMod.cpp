...
void tcp_connection::start(boost::asio::io_context& io_context, boost::lockfree::queue<RAC::Frame*>& queue, boost::lockfree::queue<RAC::InputEvent*>& inputEventQueue)
{
    try {
...
void tcp_connection::readThread(boost::lockfree::queue<RAC::InputEvent*>& inputEventQueue) {
...
void tcp_connection::writeThread(boost::asio::io_context& io_context, boost::lockfree::queue<RAC::Frame*>& queue) {
    socket_.wait_read();
    int l = 0;
    ...
    while (!shouldStop)
    {
        RAC::Frame* f;
        if (queue.pop(f)) {
            CompressionModule::EncodeLZ4(f);//EncodePNG(f);
...
tcp_server::tcp_server(boost::asio::io_context& io_context, boost::lockfree::queue<RAC::Frame*>& frameQueue, boost::lockfree::queue<RAC::InputEvent*>& inputEventQueue, bool& shouldStop, boost::function<void()> onConnectCallback, boost::function<void()> onDisconnectCallback)
